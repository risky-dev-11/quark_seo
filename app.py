...
def show_results(url):
    ...
    # make sure the given URL ends with a trailing slash
    if not url.endswith('/'):
        url += '/'
    """
    table = db.table(url)
...
@app.route('/overall_results/<path:url>', methods=['GET'])
def get_overall_results(url):
     # Process the URL (url will be automatically URL-decoded)
    if not url.endswith('/'):
        url += '/'   
    table = db.table(url)
...
@app.route('/general_results/<path:url>', methods=['GET'])
...
@app.route('/metadata_results/<path:url>', methods=['GET'])
...
@app.route('/pagequality_results/<path:url>', methods=['GET'])
...
@app.route('/pagestructure_results/<path:url>', methods=['GET'])
...
@app.route('/links_results/<path:url>', methods=['GET'])
...
@app.route('/server_results/<path:url>', methods=['GET'])
...
@app.route('/externalfactors_results/<path:url>', methods=['GET'])
